...
de(de), width(width), height(height), lastTime(0.0), speed(500), score(0)
{
    srand(time(0));
    ...
    // Initialize the current, hold and next pieces
    current = 0;
	hold = 0;
...
void Level::timerUpdate()
{
    ...
    {
        int lines = clearRows();
        speed = max(speed - 5 * lines, 50);	
		score += calScore(lines);
		line += lines;
        dropRandomPiece();
        drawScore();
        drawSpeed();
...
int Level::calScore(int lines)
{
	...
	case 3: return result += 9;
	case 4: return result += 16;
	}
	return result;
}
...
void Level::drawLine() const
{
	de.drawLine(line, width + 1, 14);
}
...
void Level::drawHoldPiece() const
{
	if(hold == 0)de.drawHoldPiece(*pieceSet.getPiece(0, 0), width + 1, 14);
	else de.drawHoldPiece(*hold, width + 1, 14);
}